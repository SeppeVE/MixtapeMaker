...
const clipId = `label-clip-${useId().replace(/:/g, '')}`;
const isUntitled = computed(() => !props.title || props.title === 'Untitled Mixtape');
...
<template>
  <div :class="`${float ? 'lp-cassette-float' : 'cassette-svg'} line-height-0`">
    <svg width="100%" viewBox="0 0 300 195" fill="none" xmlns="http://www.w3.org/2000/svg" class="cassette-drop-shadow block">
      <!-- Shell -->
      ...
      <!-- Left grippers -->
      <circle v-for="(cy, i) in beads" :key="i" cx="10" :cy="cy" r="4" fill="#2A1E28" />
      ...
      <template v-else>
        ...
        <text x="66" y="43" font-family="monospace" font-size="9" font-weight="700" fill="#2A1E28" :clip-path="`url(#${clipId})`">{{ title }}</text>
      </template>
      ...
      <!-- Tape line - animated -->
      <path d="M 84 97 Q 150 85 216 97" stroke="#3a2a38" stroke-width="3" fill="none" class="lp-tape-line" />
      ...
      <!-- Chin holes -->
      <circle cx="86" cy="168" r="6" fill="rgba(255,255,255,1)" stroke="#2A1E28" stroke-width="1.5" />
      <circle cx="118" cy="169" r="6" fill="rgba(255,255,255,1)" stroke="#2A1E28" stroke-width="1.5" />
      ...
      <circle cx="182" cy="169" r="6" fill="rgba(255,255,255,1)" stroke="#2A1E28" stroke-width="1.5" />
      <circle cx="214" cy="168" r="6" fill="rgba(255,255,255,1)" stroke="#2A1E28" stroke-width="1.5" />

      <!-- Corner screws - drawn last so they sit on top -->
      <g v-for="([cx, cy], i) in screws" :key="'s' + i">
        <circle :cx="cx" :cy="cy" r="7" fill="#DDD2B8" stroke="#2A1E28" stroke-width="1.5" />
        <text :x="cx" :y="cy + 4" text-anchor="middle" font-size="9" font-weight="bold" fill="#2A1E28">+</text>
      </g>
    </svg>
  </div>
</template>
