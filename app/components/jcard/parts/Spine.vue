...
const topStyle = computed<CSSProperties>(() => ({
  ...base,
  top: '1.5mm',
  transformOrigin: props.content.isReversed ? 'right center' : 'left center',
  transform: props.content.isReversed ? `translateX(-100%) ${rot.value}` : `translateX(0) ${rot.value}`,
}));
const midStyle = computed<CSSProperties>(() => ({
  ...base,
  top: '50%',
  transformOrigin: 'center center',
  transform: `translateX(-50%) translateY(-50%) ${rot.value}`,
}));
const btmStyle = computed<CSSProperties>(() => ({
  ...base,
  bottom: '1.5mm',
  transformOrigin: props.content.isReversed ? 'left center' : 'right center',
  transform: props.content.isReversed ? `translateX(0) ${rot.value}` : `translateX(-100%) ${rot.value}`,
}));
</script>
