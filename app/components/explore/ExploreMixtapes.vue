...
// Restore state from the URL (q/page) so the back button from a tape's
...
// which is what search engines see.
const { data: initial } = await useAsyncData('explore-mixtapes-initial', async () => {
  const result = await searchPublicMixtapes(initialQuery, PAGE_SIZE, (initialPage - 1) * PAGE_SIZE);
...
function syncUrl() {
  router.replace({ query: { q: query.value.trim() || undefined, page: page.value > 1 ? String(page.value) : undefined } });
}
