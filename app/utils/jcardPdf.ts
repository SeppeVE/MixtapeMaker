...
export async function exportJCardToPDF(content: JCardContent, filename = 'jcard') {
  ...
  // Mount the printable Vue component into the detached host, snapshot, teardown.
  let app: ReturnType<typeof createApp> | null = null;
  try {
    app = createApp({ render: () => h(JCardPrintable, { content }) });
    app.mount(host);
    await twoFrames();
    ...
    if (hasInsideContent) {
      ...
      let insideApp: ReturnType<typeof createApp> | null = null;
      try {
        insideApp = createApp({ render: () => h(JCardInsidePrintable, { content }) });
        insideApp.mount(insideHost);
        await twoFrames();
        if ((document as any).fonts?.ready) {
          try { await (document as any).fonts.ready; } catch { /* ignore */ }
        }
        ...
        try { insideApp?.unmount(); } catch { /* ignore */ }
